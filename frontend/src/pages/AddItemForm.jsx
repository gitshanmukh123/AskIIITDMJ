  ...
  <div>
    ...
    <div className="relative mt-1">
      ...
      <input
        ...
        className={`w-full ${icon ? "pl-10" : ""} px-4 py-2 rounded-lg border outline-none transition
        ${
          isDark
    ...
    >
      ...
      <div className="space-y-1">
        <h2 className="text-xl sm:text-2xl md:text-3xl font-bold">
          Post <span className="text-blue-500">Lost/Found</span>
        </h2>
        <p className={isDark ? "text-blue-200/80 text-xs sm:text-sm" : "text-slate-600 text-xs sm:text-sm"}>
          Help return campus items
        </p>
      </div>

      {/* TYPE */}
      <div className="flex gap-2 sm:gap-3">
        <ToggleBtn active={type === "lost"} label="Lost" color="red" onClick={() => setType("lost")} isDark={isDark} />
        <ToggleBtn active={type === "found"} label="Found" color="green" onClick={() => setType("found")} isDark={isDark} />
      </div>

      {/* TITLE */}
      <Input label="Title" value={title} onChange={setTitle} placeholder="Black bag" required isDark={isDark} />
      ...
      <div>
        ...
        <textarea
          ...
          className={`w-full mt-1 px-4 py-2 rounded-lg border outline-none transition
          ${
            isDark
              ? "bg-slate-900/70 border-blue-500/20 text-white focus:border-blue-400"
              : "bg-white border-slate-300 text-slate-900 focus:border-blue-500"
      ...
      <div>
        ...
        <div className="relative mt-1">
          ...
          <select
            ...
            className={`w-full pl-10 px-4 py-2 rounded-lg border outline-none
            ${
              isDark
      ...
      {/* LOCATION */}
      <Input label="Place" value={location} onChange={setLocation} placeholder="Where?" icon={<MapPin size={16} />} required isDark={isDark} />
