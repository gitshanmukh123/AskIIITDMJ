...
const TopicForm = ({ loading, setLoading, setResult, setError }) => {
  ...
  useEffect(() => {
    ...
    const steps = [
      ...
      { p: 35, t: "Structuring notes..." },
      { p: 55, t: "Finding key points..." },
      { p: 75, t: "Adding visuals..." },
      { p: 90, t: "Finishing..." }
    ]
    ...
    >
      ...
      <div className="mb-4">
        ...
        <input
          ...
          placeholder="Web dev"
          className={`w-full mt-1 px-4 py-2 rounded-lg border outline-none focus:border-blue-500 ${inputBg}`}
        />
      </div>

      {/* CLASS LEVEL */}
      <div className="mb-4">
        <label className={`text-sm ${labelText}`}>Level</label>
        <select
          value={classLevel}
      ...
      <div className="space-y-3 mb-6">
        <Toggle label="Revision" value={revisionMode} onChange={setRevisionMode} isDark={isDark} />
        <Toggle label="Diagrams" value={includeDiagram} onChange={setIncludeDiagram} isDark={isDark} />
        <Toggle label="Charts" value={includeChart} onChange={setIncludeChart} isDark={isDark} />
      </div>
